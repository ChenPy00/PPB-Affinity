...
class AddAtomNoise(object):
    ...
    def __call__(self, data):
        # LHQ: 所有pos_atoms替换为pos_heavyatom， mask_atoms替换为mask_heavyatom
        pos_atoms = data['pos_heavyatom']#data['pos_atoms']   # (L, A, 3)
        mask_atoms = data['mask_heavyatom']#data['mask_atoms'] # (L, A)
        noise = (torch.randn_like(pos_atoms) * self.noise_std) * mask_atoms[:, :, None]
        pos_noisy = pos_atoms + noise
        data['pos_heavyatom'] = pos_noisy
        return data
